...
function renderFieldType(field: Field) {
	...
	let fieldTypesFlat =
		fieldTypes.length > 1 ? fieldTypes.join(`|`) : fieldTypes[0];
...
function renderFields(renderQueue: Field[] = []): string {
	...
	while (renderQueue.length) {
		...
		let serialiedFieldMap = `interface ${
			hasInterfaceTypes(fieldMap) && fieldMap.interfaceName
		} {`;
